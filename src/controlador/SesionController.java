...
public class SesionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        DataSistema.usuarios();
        DataSistema.productos();
        DataSistema.clientes();
        DataSistema.ordenes();
        
        clientes = new ArrayList();
        for (Cliente c : DataSistema.todos) {
            clientes.add(c);
        }
        productos = new ArrayList();
        for (Producto p : DataSistema.productos) {
            productos.add(p);
        }
        ordenes = new ArrayList();
        for (Orden o : DataSistema.orden) {
            ordenes.add(o);
        }
    }
    ...
    private void login(ActionEvent event) {
        ...
        if (valida == true) {
            ...
            JOptionPane.showMessageDialog(null, "Invalido", "Error", JOptionPane.INFORMATION_MESSAGE);
            txtPassword.setText("");
            txtUser.setText("");
        }
